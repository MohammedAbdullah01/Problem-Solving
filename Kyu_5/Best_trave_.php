...
function chooseBestSum($t, $k, $ls)
{
    ...
    foreach ($combinations as $towns) {
        $sum = array_sum($towns);
        if ($sum <= $t && ($best_sum === null || $sum > $best_sum)) {
            $best_sum = $sum;
        }
    }
    return $best_sum;
...
function combinations($arr, $k)
{
    ...
    $combinations = [];
    foreach (combinations($rest, $k - 1) as $sub_comb) {
        array_unshift($sub_comb, $first);
        $combinations[] = $sub_comb;
    }
    foreach (combinations($rest, $k) as $sub_comb) {
        $combinations[] = $sub_comb;
    }
...
$xs = [50];
echo chooseBestSum(163, 3, $xs) . "\n";  // Output: null

$ys = [91, 74, 73, 85, 73, 81, 87];
echo chooseBestSum(230, 3, $ys) . "\n";  // Output: 228